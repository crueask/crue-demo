...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, to, projectName, inviterEmail, role, token, isExistingUser } = body;

    // Accept either 'email' or 'to' field
    const recipientEmail = email || to;

    // For existing users, token is not required
    if (!recipientEmail || !projectName || (!token && !isExistingUser)) {
    ...
    if (isExistingUser) {
      // Email for existing users - they already have access, just notify them
      const dashboardUrl = `${baseUrl}/dashboard`;
      emailSubject = `Du har fått tilgang til ${projectName} på Crue`;
      emailHtml = `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #10b981, #059669); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">Ny prosjekttilgang!</h1>
          </div>
          ...
          <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
            ...
            <p style="font-size: 16px; margin-bottom: 20px;">
              ${inviterEmail ? `<strong>${inviterEmail}</strong> har gitt` : "Du har fått"} deg tilgang til <strong>${projectName}</strong> på Crue.
            </p>
            ...
            <p style="font-size: 16px; margin-bottom: 20px;">
              Du har allerede en konto på Crue, så prosjektet er nå tilgjengelig for deg.
            </p>
      ...
      emailSubject = `Du er invitert til ${projectName} på Crue`;
      emailHtml = `
        <!DOCTYPE html>
        <html>
        <head>

...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, organizationName, inviterEmail, role, token, isExistingUser } = body;

    if (!email || !organizationName || (!token && !isExistingUser)) {
    ...
    if (isExistingUser) {
      // Email for existing users - they already have access
      const dashboardUrl = `${baseUrl}/dashboard/settings`;
      emailSubject = `Du er lagt til i ${organizationName} på Crue`;
      emailHtml = `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #10b981, #059669); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">Ny organisasjonstilgang!</h1>
          </div>
          ...
          <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
            ...
            <p style="font-size: 16px; margin-bottom: 20px;">
              ${inviterEmail ? `<strong>${inviterEmail}</strong> har lagt` : "Du har blitt lagt"} deg til i <strong>${organizationName}</strong> på Crue.
            </p>

            <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #e5e7eb; margin-bottom: 20px;">
              <p style="margin: 0 0 8px 0; font-size: 14px; color: #6b7280;">Din rolle:</p>
              <p style="margin: 0; font-size: 16px; font-weight: 600;">${roleLabel}</p>
            </div>

            <p style="font-size: 16px; margin-bottom: 20px;">
              Du har allerede en konto på Crue, så organisasjonen er nå tilgjengelig for deg.
            </p>

            <div style="text-align: center; margin: 30px 0;">
              <a href="${dashboardUrl}" style="display: inline-block; background: #10b981; color: white; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 16px;">
          ...
          <div style="text-align: center; padding: 20px;">
            <p style="font-size: 12px; color: #9ca3af; margin: 0;">
              Sendt fra <a href="https://crue.no" style="color: #10b981;">Crue</a>
            </p>
          </div>
        </body>
        </html>
      ...
      // Email for new users - they need to accept the invitation
      const inviteUrl = `${baseUrl}/org-invite/${token}`;
      emailSubject = `Du er invitert til ${organizationName} på Crue`;
      emailHtml = `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
            ...
            <p style="font-size: 16px; margin-bottom: 20px;">
              ${inviterEmail ? `<strong>${inviterEmail}</strong> har` : "Du har blitt"} invitert deg til å bli med i <strong>${organizationName}</strong> på Crue.
            </p>

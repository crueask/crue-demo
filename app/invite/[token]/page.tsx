...
import { Check, X, Clock, AlertCircle, LogIn } from "lucide-react";
...
export default function InvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  function goToProject() {
    if (acceptedProjectId) {
      router.push(`/dashboard/projects/${acceptedProjectId}`);
    } else {
        ...
        <Card className="w-full max-w-md">
          <CardContent className="py-8 text-center">
            ...
            <p className="text-gray-500 mb-6">
              Du har nå tilgang til {invitation?.projectName}.
            </p>
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-600 mb-2">Invitasjon til:</p>
              <p className="font-medium text-gray-900">{invitation?.projectName}</p>
              <div className="flex items-center gap-2 mt-2">
                <span className="text-sm text-gray-500">Tilgang:</span>
                <UserAccessBadge role={invitation?.role || "viewer"} />
              </div>
            </div>
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Du har fått tilgang til et prosjekt på Crue.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="p-4 bg-gray-50 rounded-lg">
            <p className="text-sm text-gray-600 mb-2">Prosjekt:</p>
            <p className="font-medium text-gray-900 text-lg">{invitation?.projectName}</p>
            <div className="flex items-center gap-2 mt-2">
              <span className="text-sm text-gray-500">Din tilgang:</span>
              <UserAccessBadge role={invitation?.role || "viewer"} showDescription />
            </div>
          </div>
